...
const servidor = new WebSocketServer({ port: process.env.PORT || 3000 });
...
servidor.on("connection", (ws) => {
      ...
      servidor.clients.forEach((client) => {
        if (client.readyState === 1) {
          ...
          client.send(
            JSON.stringify([
              datos_recibidos[0],
              datos_recibidos[1].toString().replace("ALLCLIENTS_LOG", "").trim(),
            ])
          );
        }
      });
  ...
  }); // <--- Faltaba cerrar el ws.on("message")
}); // <--- Faltaba cerrar el servidor.on("connection")
